...
function activate(activating)
    ...
    if activating then
		set_date_text()
		obs.timer_add(timer_callback, 10000)
	else
...
function script_description()
    return "Sets a text source to the current date when the source is active.\n\nFor formatting, see https://www.lua.org/pil/22.1.html\n\n - by DJC"
end
...
function script_defaults(settings)
    obs.obs_data_set_default_string(settings, "format", "%b %d, %Y")
end
